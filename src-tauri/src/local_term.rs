use portable_pty::{CommandBuilder, NativePtySystem, PtyPair, PtySize, PtySystem, Child};
...
impl LocalTerminalManager {
    ...
    pub fn create_terminal(&self, id: String, rows: u16, cols: u16, app_handle: AppHandle) -> Result<()> {
        ...
        let pair = pty_system.openpty(PtySize {
            ...
            pixel_width: 0,
            pixel_height: 0,
        }).map_err(|e| crate::error::SshError::Io(std::io::Error::new(std::io::ErrorKind::Other, e)))?;
        ...
        self.sessions.lock().unwrap().insert(id, TerminalSession { pair, child, writer });
        Ok(())
    }
