...
- ⚡ **高性能终端** - 基于 xterm.js，支持历史回滚、搜索、分屏和断线按键重连
- 💻 **本地终端** - 集成本地 Shell 支持 (PowerShell/Bash)
- 📁 **远程文件管理** - 支持 SFTP 文件管理、拖拽上传、内联进度和安全删除确认
- 📝 **代码编辑器** - 内置 Monaco Editor (VS Code 内核)，支持语法高亮
- 📊 **系统监控** - 实时监控 CPU、内存、磁盘和网络状态
- 🎯 **命令片段** - 常用命令的快速保存与执行
- ⌨️ **快捷键可发现性** - 可配置快捷键、`Ctrl+/` 快捷键总览和分屏切换
...
在已连接的远程终端处拖入本地文件时，应用会自动打开 **SFTP** 侧栏，并上传到该连接当前所在的 SFTP 文件夹。左侧文件列表会显示进度；完成后会短暂保留完整文件名，再刷新目录。

上传目标不会根据终端提示符推断。例如，即使终端执行过 `cd /data/product`，也不会自动上传到该目录：提示符可能被自定义，不能可靠反映 Shell 当前路径。请先在 SFTP 文件管理器中进入 `/data/product`；若尚未打开过 SFTP 文件夹，则默认上传到远程用户主目录。
...
### 🎨 现代化终端
基于 xterm.js 的高性能终端体验，支持自定义主题、字体调节、多标签与分屏、终端搜索，以及断线后按任意键重连。
