export default {
  common: {
    settings: 'Settings',
    save: 'Save',
    cancel: 'Cancel',
    language: 'Language',
    theme: 'Theme',
    loading: 'Loading...',
    error: 'Error',
    delete: 'Delete',
    edit: 'Edit',
    reset: 'Reset',
    connect: 'Connect',
    disconnect: 'Disconnect',
    confirm: 'Confirm',
    saving: 'Saving...',
    update: 'Update',
    add: 'Add',
    saved: 'Saved',
    save_failed: 'Save failed',
    deleted: 'Deleted',
    delete_failed: 'Delete failed',
    copy: 'Copy',
    select_all: 'Select All',
    clear: 'Clear',
    paste: 'Paste',
    copied_to_clipboard: 'Copied to clipboard',
    copy_failed: 'Copy failed',
    paste_failed: 'Paste failed',
    execute: 'Execute',
    close: 'Close',
    all: 'All',
    optional: '(Optional)',
    new_local_terminal: 'New Local Terminal',
    select_shell: 'Select shell',
    default: 'Default',
    minimize: 'Minimize',
    maximize: 'Maximize',
    restore: 'Restore',
    export: 'Export',
    exported: 'Exported!',
    search: 'Search',
    no_results: 'No results',
    hosts: 'Hosts',
    sftp: 'SFTP',
    snippets: 'Snippets',
    select: 'Select',
    load: 'Load',
    local_terminal: 'Local Terminal',
    press_key_to_connect: 'Press {{keys}} to connect',
    no_active_connection: 'No active connection',
    select_server_first: 'Select a server to connect first',
    close_tab: 'Close Tab',
    browse: 'Browse',
    starting: 'Starting...',
    retry: 'Retry',
    keyboard_shortcuts: 'Keyboard Shortcuts',
    close_other_tabs: 'Close Other Tabs',
    close_all_tabs: 'Close All Tabs',
    close_all_tabs_confirm: 'Close all open tabs? Unsaved changes will be lost.',
  },
  update: {
    title: 'Update Available',
    available_msg: 'A new version {{version}} is available!\n\nRelease notes:\n{{body}}',
    available_msg_simple: 'A new version {{version}} is available. Do you want to update now?',
    update_now: 'Update Now',
    cancel: 'Later',
    checking: 'Checking for updates...',
    uptodate: 'You are using the latest version.',
    error: 'Failed to check for updates: {{error}}',
    downloading: 'Downloading update…',
    preparing: 'Preparing…',
  },
  transfer: {
    transferring: 'Transferring ({{count}})',
    completed: 'Completed',
    complete: 'Transfer complete',
    failed: 'Transfer failed',
  },
  server: {
    add: 'Add Server',
    edit: 'Edit Server',
    list: 'Servers',
    name: 'Name',
    host: 'Host',
    port: 'Port',
    username: 'Username',
    password: 'Password',
    key: 'Key',
    no_servers: 'No servers',
    delete_confirm: 'Are you sure you want to delete this server?',
    passphrase: 'Passphrase (optional)',
    private_key_path: 'Private Key Path (optional)',
    password_optional: 'Password (optional)',
    test_success: 'Connection successful!',
    test_failed: 'Connection failed or rejected.',
    password_placeholder: 'Leave empty for key auth',
    auth_method: 'Auth Method',
    auth_password: 'Password',
    auth_private_key: 'Private Key',
    browse: 'Browse...',
  },
  terminal: {
    welcome: 'Welcome to HeTaoSSH',
    start_tip: 'Connect to a server from the left sidebar to start.',
    disconnected: 'Disconnected',
    connecting: 'Connecting to server...',
    show_panel: 'Show panel',
    hide_panel: 'Hide panel',
    search_placeholder: 'Search in terminal...',
    search_previous: 'Previous match',
    search_next: 'Next match',  
    match_found: 'Match',
    matches_found: '{{current}}/{{total}} matches',
    no_match: 'No match',
  },
  snippets: {
    title: 'Snippets',
    load_failed: 'Failed to load snippets',
    validation_error: 'Name and command are required',
    delete_confirm: 'Delete snippet "{{name}}"?',
    search_placeholder: 'Search name/command/description...',
    add: 'Add Snippet',
    edit: 'Edit Snippet',
    no_match: 'No matching snippets',
    name: 'Name',
    name_placeholder: 'e.g., Disk Usage',
    category: 'Category',
    category_placeholder: 'e.g., System (optional)',
    command: 'Command',
    command_placeholder: 'e.g., df -h',
    description: 'Description',
    saved: 'Saved',
    save_failed: 'Failed to save: {{error}}',
    delete: 'Delete',
    deleted: 'Deleted',
    delete_success: 'Deleted successfully',
    delete_failed: 'Failed to delete: {{error}}',
    copy: 'Copy',
    execute: 'Execute',
    close: 'Close',
    optional: '(Optional)',
},
  file: {
    explorer: 'Explorer',
    no_file_selected: 'No file selected',
    select_tip: 'Select a file from the tree to edit',
    loading: 'Loading file...',
    loading_editor: 'Loading editor...',
    error: 'Error loading file',
    show_hidden: 'Show Hidden Files',
    hide_hidden: 'Hide Hidden Files',
    refresh: 'Refresh',
    open_folder: 'Open Folder',
    open_folder_failed: 'Failed to open folder',
    select_folder: 'Select Folder',
    enter_remote_path: 'Enter remote path',
    select: 'Select',
    connect_to_view: 'Connect to a server to view files.',
    sidebar_close: 'Close sidebar',
    sidebar_open: 'Open sidebar',
    save_success: 'File saved successfully',
    save_failed: 'Failed to save file: {{error}}',
    load_failed: 'Failed to load file: {{error}}',
    drop_dir_skipped: 'Skipped folder: {{name}} (only files can be dropped)',
    unsaved_changes: 'Unsaved',
    unsaved_close_confirm: '"{{name}}" has unsaved changes that will be lost. Close anyway?',
    copy_path: 'Copy Path',
    open_in_explorer: 'Open in Explorer',
    open_failed: 'Failed to open',
    new_file: 'New File',
    new_folder: 'New Folder',
    new_file_name: 'Enter file name:',
    new_folder_name: 'Enter folder name:',
    create_file_success: 'File created',
    create_file_failed: 'Failed to create file',
    create_folder_success: 'Folder created',
    create_folder_failed: 'Failed to create folder',
    moving: 'Moving {{name}}...',
    move_success: '{{name}} moved successfully',
    move_failed: 'Failed to move {{name}}: {{error}}',
    download: 'Download',
    download_save_as: 'Save file as',
    download_started: 'Download started...',
    download_success: 'File/Directory downloaded successfully',
    download_failed: 'Failed to download file',
    download_dir_not_supported: 'Directory download not supported',
    download_dir_error: 'Cannot download directory (or symlink to one)',
    select_download_folder: 'Select download destination',
    select_download_dir: 'Select download directory',
    delete: 'Delete',
    delete_confirm: 'Are you sure you want to delete this file?',
    delete_success: 'File deleted successfully',
    delete_failed: 'Failed to delete file',
    upload_failed_no_path: 'Unable to resolve file path',
    uploading: 'Uploading {{name}}...',
    uploading_files: 'Uploading',
    path_placeholder: 'Enter path or select from browser below',
    upload_success: '{{name}} uploaded successfully',
    upload_failed: '{{name}} upload failed: {{error}}',
  },
  settings: {
    settings: 'Settings',
    help: 'Show Keyboard Shortcuts',
    title: 'Settings',
    appearance: 'Appearance',
    editor: 'Editor',
    minimap: 'Minimap',
    word_wrap: 'Word Wrap',
    font_size: 'Font Size',
    line_height: 'Line Height',
    theme_select: 'Select Theme',
    import_theme: 'Import Theme',
    import_placeholder: 'Import Local File (.json/.itermcolors)',
    url_placeholder: 'Enter Theme URL (e.g. GitHub Raw)',
    import_url: 'URL Import',
    theme_imported: 'Theme imported successfully!',
    theme_import_failed: 'Failed to import theme: {{error}}',
    url_import_failed: 'Failed to download theme: {{error}}',
    theme_exists: 'Theme "{{name}}" already exists',
    delete_theme_confirm: 'Are you sure you want to delete theme "{{name}}"?',
    html_content_error: 'HTML content detected. Please ensure you use the "Raw" file URL (GitHub links are auto-converted).',
    paste_json: 'Paste JSON',
    paste_placeholder: 'Paste theme JSON content here...',
    import: 'Import',
    invalid_json: 'Invalid JSON format',
    invalid_format_error: 'Invalid theme format',
    theme_market: {
      title: 'Theme Market',
      import: 'Import Theme',
      no_custom: 'No custom themes yet',
      import_hint: 'Import a theme file to get started',
      custom: 'Custom Theme',
    },
    right_click: 'Right Click',
    behavior_menu: 'Context Menu',
    behavior_paste: 'Paste',
    smart_behavior: 'Smart behavior: Right click to paste (or copy if selection exists).',
    standard_behavior: 'Standard behavior: Right click shows context menu.',
    terminal: 'Terminal',
    mouse: 'Mouse',
  },
  status: {
    not_connected: 'Not connected',
    connected: 'Connected',
    connecting: 'Connecting...',
    disconnected: 'Disconnected',
    reconnect: 'Reconnect',
    press_any_key_to_reconnect: 'Press any key to reconnect',
    reconnecting: 'Reconnecting...',
    monitor: 'Monitor',
    refresh_rate: '3s refresh',
    cpu: 'CPU',
    memory: 'Memory',
    network: 'Network',
    network_traffic: 'Network Traffic',
    network_speed: 'Network Speed',
    total_traffic: 'Total Traffic',
    uptime: 'Uptime',
    load_average: 'Load Avg',
    download: 'Download',
    upload: 'Upload',
    disk: 'Disk',
    loading: 'Loading...',
    fetch_failed: 'Fetch failed: {{error}}',
    no_data: '(No data)',
    disk_more: '{{count}} more items...',
    usage: 'Usage',
    used: 'Used',
    total: 'Total',
    avail: 'Avail',
    received: 'Received',
    transmitted: 'Transmitted',
    disk_usage: 'Disk Usage',
    available: 'Available',
    system_info_loading: 'Loading system info...',
    system_info_hint: 'Connect to a server to view system info.',
  },
  file_tree: {
    path_placeholder: 'Enter path, e.g. /var/log',
    load_error: 'Failed to load directory: {{error}}',
    loading: 'Loading...',
    connecting: 'Connecting...',
    root: 'Root',
  },
  store: {
    load_servers_failed: 'Failed to load servers: {{error}}',
    save_server_failed: 'Failed to save server: {{error}}',
    delete_server_failed: 'Failed to delete server: {{error}}',
    connection_test_failed: 'Connection test failed: {{error}}',
    server_not_found: 'Server not found: {{id}}',
    connect_failed: 'Failed to connect: {{error}}',
    disconnect_failed: 'Failed to disconnect: {{error}}',
    send_data_failed: 'Failed to send data: {{error}}',
    auth_failed: 'Authentication failed: Username "{{username}}" cannot connect to {{host}}:{{port}}. Please check:\n1. Username is correct\n2. Password or private key is valid\n3. Server allows this authentication method',
  },
  shortcuts: {
    title: 'Keyboard Shortcuts',
    new_connection: 'New Connection',
    toggle_sidebar: 'Toggle Sidebar',
    close_tab: 'Close Tab',
    new_local_terminal: 'New Local Terminal',
    settings: 'Settings',
    terminal_search: 'Terminal Search',
    split_horizontal: 'Split Horizontal',
    split_vertical: 'Split Vertical',
    focus_next_pane: 'Focus Next Pane',
    close_pane: 'Close Pane',
    press_keys: 'Press keys...',
    reset_all: 'Reset all shortcuts to defaults',
     editor_save: 'Save File',
  },
  tunnel: {
    title: 'Port Forwarding',
    mode: 'Mode',
    local: 'Local Forward',
    dynamic: 'SOCKS Proxy',
    local_port: 'Local Port',
    remote_host: 'Remote Host',
    remote_port: 'Remote Port',
    start: 'Start Tunnel',
    list: 'List Active Tunnels',
    active: 'Active Tunnels',
    usage: 'Usage:',
    tip_local: 'Local: Access remote service via localhost',
    tip_socks: 'SOCKS: Use as proxy in browser/terminal',
  },
};
